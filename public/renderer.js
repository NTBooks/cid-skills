// API shim: replaces Electron IPC with REST fetch calls
window.electronAPI = {
  getFiles: () => fetch('/api/files').then(r => r.json()),
  saveFile: async (fileData, zipBuffer) => {
    let body;
    if (zipBuffer) {
      const base64 = typeof zipBuffer === 'string' ? zipBuffer
        : btoa(String.fromCharCode(...new Uint8Array(zipBuffer instanceof ArrayBuffer ? zipBuffer : zipBuffer.buffer || zipBuffer)));
      body = JSON.stringify({ fileData, zipBuffer: base64 });
    } else {
      body = JSON.stringify({ fileData });
    }
    return fetch('/api/files', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body }).then(r => r.json());
  },
  readFile: (cid) => fetch(`/api/files/${encodeURIComponent(cid)}`).then(r => r.json()),
  deleteFile: (cid) => fetch(`/api/files/${encodeURIComponent(cid)}`, { method: 'DELETE' }).then(r => r.json()),
  updateFileTags: (cid, tags) => fetch(`/api/files/${encodeURIComponent(cid)}/tags`, {
    method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tags })
  }).then(r => r.json()),
  calculateHash: async (content) => {
    let body;
    if (content instanceof ArrayBuffer || (content && content.buffer instanceof ArrayBuffer)) {
      const bytes = new Uint8Array(content instanceof ArrayBuffer ? content : content.buffer);
      const base64 = btoa(String.fromCharCode(...bytes));
      body = JSON.stringify({ content: { type: 'Buffer', data: Array.from(bytes) } });
    } else {
      body = JSON.stringify({ content });
    }
    return fetch('/api/hash', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body }).then(r => r.json());
  },
  fetchDsoulByCid: (cid) => fetch(`/api/dsoul/${encodeURIComponent(cid)}`).then(r => r.json()),
  ...
  getBundleLicenseContent: (cid) => fetch(`/api/files/${encodeURIComponent(cid)}/bundle/license`).then(r => r.json()),
  validateZipSkillBundle: async (buffer) => {
    const bytes = new Uint8Array(buffer instanceof ArrayBuffer ? buffer : buffer.buffer || buffer);
    const base64 = btoa(String.fromCharCode(...bytes));
    return fetch('/api/validate-zip-bundle', {
  ...
  execCommand: async function* (command, args) {
    ...
    try {
      ...
      yield { type: 'err', data: 'Network error: ' + e.message };
      yield { type: 'exit', code: 1 };
      return;
    }
    if (!resp.ok || !resp.body) {
      const err = await resp.json().catch(() => ({ error: 'Request failed' }));
      yield { type: 'err', data: err.error || 'Request failed' };
      yield { type: 'exit', code: 1 };
      return;
    }
...
const CLI_COMMANDS = [
  ...
  },
  {
    ...
    desc: 'Check for and apply upgrades',
    fields: [
      { id: 'global', label: 'Global only (-g)', type: 'checkbox' },
      { id: 'local', label: 'Local only (--local)', type: 'checkbox' },
      { id: 'deleteBlocked', label: 'Blocked items', type: 'select', options: ['ask', 'delete', 'keep'] },
    ],
    buildArgs(v) {
      const a = [];
      if (v.global) a.push('-g');
      if (v.local) a.push('--local');
      if (v.deleteBlocked === 'delete') a.push('--delete-blocked');
      else if (v.deleteBlocked === 'keep') a.push('--no-delete-blocked');
      return a;
    },
  },
  {
    id: 'upgrade', label: 'upgrade',
    desc: 'Upgrade skills to latest version',
    fields: [
      { id: 'global', label: 'Global only (-g)', type: 'checkbox' },
      { id: 'local', label: 'Local only (--local)', type: 'checkbox' },
      { id: 'auto', label: 'Auto-pick oldest (-y)', type: 'checkbox' },
  ...
  {
    id: 'init', label: 'init',
    desc: 'Create a folder with skill.md template',
    fields: [
      { id: 'directory', label: 'Directory', type: 'text', placeholder: 'path/to/new-skill', required: true },
    ],
    buildArgs(v) { return v.directory ? [v.directory] : []; },
  },
  {
    id: 'package', label: 'package',
    desc: 'Package a folder as a zip bundle',
    fields: [
      { id: 'folder', label: 'Folder', type: 'text', placeholder: 'path/to/skill-folder', required: true },
    ],
    buildArgs(v) { return v.folder ? [v.folder] : []; },
  },
  {
    id: 'hash', label: 'hash cidv0',
    desc: 'Print CIDv0 (IPFS) hash of a file',
    fields: [
      { id: 'file', label: 'File', type: 'text', placeholder: 'path/to/file.zip', required: true },
    ],
    buildArgs(v) { return v.file ? ['cidv0', v.file] : []; },
  },
  {
    id: 'freeze', label: 'freeze',
    desc: 'Stamp a file via the DSOUL freeze API',
    fields: [
      { id: 'file', label: 'File', type: 'text', placeholder: 'path/to/file.zip', required: true },
      { id: 'shortname', label: 'Shortname', type: 'text', placeholder: 'user@name:1.0.0 (optional)' },
      { id: 'tags', label: 'Tags', type: 'text', placeholder: 'tag1,tag2 (optional)' },
      { id: 'version', label: 'Version', type: 'text', placeholder: '1.0.0 (optional)' },
      { id: 'license_url', label: 'License URL', type: 'text', placeholder: 'https://… (optional)' },
    ],
    buildArgs(v) {
      const a = [];
      if (v.file) a.push(v.file);
      if (v.shortname) a.push('--shortname=' + v.shortname);
      if (v.tags) a.push('--tags=' + v.tags);
      if (v.version) a.push('--version=' + v.version);
      if (v.license_url) a.push('--license_url=' + v.license_url);
      return a;
    },
  },
  {
    id: 'balance', label: 'balance',
    desc: 'Check stamp/credit balance',
    fields: [],
    buildArgs() { return []; },
  },
  {
    id: 'files', label: 'files',
    desc: 'List your frozen files',
    fields: [
      { id: 'page', label: 'Page', type: 'text', placeholder: '1' },
      { id: 'per_page', label: 'Per page', type: 'text', placeholder: '100' },
    ],
    buildArgs(v) {
      const a = [];
      if (v.page && v.page !== '1') a.push('--page=' + v.page);
      if (v.per_page && v.per_page !== '100') a.push('--per_page=' + v.per_page);
      return a;
    },
  },
  {
    id: 'register', label: 'register',
    desc: 'Store WordPress credentials',
    fields: [
      { id: 'username', label: 'Username', type: 'text', placeholder: 'WordPress username (optional)' },
      { id: 'token', label: 'Token', type: 'text', placeholder: 'Application password (optional)' },
    ],
    buildArgs(v) {
      const a = [];
      if (v.username) a.push(v.username);
      if (v.token) a.push(v.token);
      return a;
    },
  },
  {
  ...
  },
  {
    ...
    desc: 'Set or view a config value',
    fields: [
      { id: 'key', label: 'Key', type: 'select', options: ['dsoul-provider', 'skills-folder', 'skills-folder-name'], required: true },
      { id: 'value', label: 'Value', type: 'text', placeholder: 'New value (leave empty to view current)' },
    ],
    buildArgs(v) {
      const a = [v.key || 'dsoul-provider'];
...
function setupCliPanel() {
  ...
  // Build sidebar command buttons
  for (const cmd of CLI_COMMANDS) {
    const btn = document.createElement('button');
    btn.className = 'cli-command-btn' + (cmd === selectedCmd ? ' active' : '');
    btn.textContent = cmd.label;
    btn.dataset.cmdId = cmd.id;
    btn.addEventListener('click', () => selectCommand(cmd));
    cliSidebar.appendChild(btn);
  }

  function selectCommand(cmd) {
    selectedCmd = cmd;
    cliSidebar.querySelectorAll('.cli-command-btn').forEach(b => {
      b.classList.toggle('active', b.dataset.cmdId === cmd.id);
    });
    renderForm(cmd);
  }

  function renderForm(cmd) {
    cliFormArea.innerHTML = '';

    const title = document.createElement('div');
    title.className = 'cli-form-title';
    title.textContent = 'dsoul ' + cmd.label;
    cliFormArea.appendChild(title);

    if (cmd.desc) {
      const desc = document.createElement('div');
      desc.className = 'cli-form-desc';
      desc.textContent = cmd.desc;
      cliFormArea.appendChild(desc);
    }

    for (const field of cmd.fields) {
      const row = document.createElement('div');
      row.className = 'cli-form-row';

      const label = document.createElement('label');
      label.textContent = field.label;
      label.htmlFor = 'cli-f-' + field.id;
      ...
      input.id = 'cli-f-' + field.id;
      input.dataset.fieldId = field.id;
      row.appendChild(input);
      cliFormArea.appendChild(row);
    }

    const actions = document.createElement('div');
    actions.className = 'cli-form-actions';
    const runBtn = document.createElement('button');
    runBtn.id = 'cli-run-btn';
    runBtn.className = 'cli-run-btn';
    runBtn.textContent = 'Run';
    runBtn.addEventListener('click', handleCliRun);
    actions.appendChild(runBtn);
    cliFormArea.appendChild(actions);
  }

  function getFormValues() {
    const v = {};
    cliFormArea.querySelectorAll('[data-field-id]').forEach(el => {
      v[el.dataset.fieldId] = el.type === 'checkbox' ? el.checked : el.value;
    });
    return v;
  }
  ...
  async function handleCliRun() {
    ...
    const args = selectedCmd.buildArgs(values);

    running = true;
    const runBtn = document.getElementById('cli-run-btn');
    if (runBtn) runBtn.disabled = true;

    const cmdDisplay = 'dsoul ' + selectedCmd.label + (args.length ? ' ' + args.join(' ') : '');
    ...
    try {
      for await (const event of window.electronAPI.execCommand(selectedCmd.id, args)) {
        if (event.type === 'out') { if (event.data !== '') appendLine(event.data, 'out'); }
          ...
          appendLine('exited ' + event.code, event.code === 0 ? 'success' : 'failure');
        }
      }
    } catch (err) {
