...
async function getViem() {
  ...
  // Prefer viem provided by an activated plugin (global.__dsoulViemProvider).
  // This avoids bundling viem into dsoul itself — the plugin supplies it.
  if (global.__dsoulViemProvider) {
    viemCache = global.__dsoulViemProvider;
    return viemCache;
  }
...
const { downloadCIDsFromEvents, downloadStampedBundleCids, downloadCIDsToAssets } = require('./ipfs-downloader');
    ...
    try {
      return await withRetry(
        async () => {
          ...
          await downloadStampedBundleCids(config);
