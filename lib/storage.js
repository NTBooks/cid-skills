...
const { getFilesDir, getSettingsPath, getNetworkSettingsPath, defaultDsoulProviderOrigin } = require('./config');
...
async function loadNetworkSettings() {
  const filePath = getNetworkSettingsPath();
  try {
    const content = await fs.readFile(filePath, 'utf-8');
    return JSON.parse(content);
  } catch (_) {
    return { allowNonLocalhost: false, adminUsername: '', adminPasswordHash: '' };
  }
}
