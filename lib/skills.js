...
function parseShortnameParts(shortname) {
  const str = String(shortname).trim();
  const atIdx = str.indexOf('@');
  const withoutAuthor = atIdx !== -1 ? str.slice(atIdx + 1) : str;
  const versionMatch = withoutAuthor.match(/^(.+?)_(\d+\.\d+.*)$/);
  if (versionMatch) return { name: versionMatch[1], version: versionMatch[2] };
  return { name: withoutAuthor, version: null };
}

function getFileSafeSkillName(fileData, shortname) {
  if (shortname && String(shortname).trim()) {
    const { name: snName, version: snVersion } = parseShortnameParts(shortname);
    const version = snVersion || fileData.skillMetadata?.version;
    const raw = version ? `${snName}-${version}` : snName;
...
async function doActivateFile(cid, options) {
  ...
  try {
    ...
    if (sameFolder) {
      ...
      const fileSafeName = getFileSafeSkillName(fileData, options && options.shortname);
