...
function parseSkillHeaderForCli(content) {
  ...
  if (frontmatterMatch) {
    ...
    const versionMatch = frontmatterMatch[1].match(/\bversion\s*:\s*(.+?)(?:\n|$)/);
    const name = nameMatch ? nameMatch[1].trim().replace(/^["']|["']$/g, '') : null;
    const version = versionMatch ? versionMatch[1].trim().replace(/^["']|["']$/g, '') : null;
    if (name || version) return { name: name || undefined, version: version || undefined };
  }
...
function getFileSafeSkillName(fileData, shortname) {
  if (shortname && String(shortname).trim()) {
    const str = String(shortname).trim();
    const atIdx = str.indexOf('@');
    const withoutAuthor = atIdx !== -1 ? str.slice(atIdx + 1) : str;
    // Convert embedded _version or :version suffix (e.g. name_0.0.1 or name:0.0.1) to name-0.0.1
    const normalized = withoutAuthor.replace(/[_:](\d+\.\d+.*)$/, '-$1');
    const hasEmbeddedVersion = normalized !== withoutAuthor;
    // Only fall back to metadata version when shortname had none
    const version = hasEmbeddedVersion ? null : fileData.skillMetadata?.version;
    const raw = version ? `${normalized}-${version}` : normalized;
    return raw.replace(/[\s\\/:*?"<>|]+/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '') || 'skill';
  }
...
async function doActivateFile(cid, options) {
  ...
  try {
    ...
    const desiredName = getFileSafeSkillName(fileData, options && options.shortname);

    let skillDir;
    let folderName;
    if (sameFolder && fileData.activatedFolderName === desiredName) {
      ...
      if (sameFolder && fileData.activatedFolderName) {
        const oldDir = path.join(baseFolder, fileData.activatedFolderName);
        try { await fs.rm(oldDir, { recursive: true, force: true }); } catch (_) {}
      }
      const result = await getSkillDirNoConflict(baseFolder, desiredName);
